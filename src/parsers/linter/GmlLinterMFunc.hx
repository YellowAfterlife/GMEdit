...
class GmlLinterMFunc {
	public static function read(oldDepth:Int, self:GmlLinter, flags:GmlLinterReadFlags) {
		...
		//
		if (flags.has(AsStat)) {
			return self.readStat(newDepth, flags.without(AsStat));
		} else return self.readExpr(newDepth, flags);
	}
}
